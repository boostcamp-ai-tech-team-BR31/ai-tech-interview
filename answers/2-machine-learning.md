...
- [피쳐(기계학습)](https://ko.wikipedia.org/wiki/%ED%94%BC%EC%B3%90_(%EA%B8%B0%EA%B3%84_%ED%95%99%EC%8A%B5))
- [머신러닝이란?](http://www.tcpschool.com/deep2018/deep2018_machine_learning)

## #26

#### 좋은 모델의 정의는 무엇일까요?

좋은 모델이란? 내가 정의한 문제에 대하여, 기존에 갖고 있던 데이터로 학습한 모델의 성능이 새로운 상황에서도 동일하게 적용되는 것이다. 그러나 정의한 문제가 상황에 따라 달라지겠지만 보편적으로는 일반화 성능이 좋아야 할것이다. 새로운 데이터가 들어와도 기존의 결과가 크게 달라지지 않아야 할것.

예시로 내가 정의한 문제가 예측의 속도나 복잡도는 전혀 신경쓰지 않고 잘 맞추는 것이 목적이라면, 내가 정의한 평가지표(ex: F1-score, Accuracy, AUC..)에  대하여, 기존의 데이터로 학습한 모델의 성능이 새로운 데이터에도 동일한 값을 갖는 경우일것이며
